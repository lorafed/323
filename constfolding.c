...
long ConstFoldPerStatement(Node* stmtNodeRight) {
    // If both left and right children are constants, then it's a candidate for constant folding.
    if (stmtNodeRight->left && stmtNodeRight->left->exprCode == CONSTANT &&
        stmtNodeRight->right && stmtNodeRight->right->exprCode == CONSTANT) {
        
        // Calculate the value of the expression
        long result = CalcExprValue(stmtNodeRight);
        
        // Replace the original expression with the computed constant value
        stmtNodeRight->exprCode = CONSTANT;
        stmtNodeRight->value = result;
        stmtNodeRight->left = NULL;
        stmtNodeRight->right = NULL;
...
void ConstFoldPerFunction(Node* funcNode) {
    ...
    while (statements != NULL) {
        ...
        if (stmt->stmtCode == ASSIGN) {
            ConstFoldPerStatement(stmt->right);
        }
